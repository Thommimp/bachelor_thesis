...
\begin{table}[htbp]
	...
	\begin{tabular}{lcccccc}
		...
		& \multicolumn{2}{c}{\textbf{All cells}} \\
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcccc}
		...
		SARIMAX - OLS      & 0.062 & [0.029, 0.098] & 3.49 & 0.0011 \\
		XGBoost - OLS    & 0.008 & [-0.018, 0.029] & 0.81 & 0.4200 \\
		XGBoost - SARIMAX  & -0.053 & [-0.08, -0.027] & 3.91 & 0.0003 \\
		
		\midrule
		\multicolumn{4}{l}{\textit{Non-seasonal cells}} \\
		SARIMAX - OLS      & 0.107 & [0.066, 0.153] & 4.62& $< 0.001$ \\
		XGBoost - OLS    & -0.040 & [-0.059, -0.021] & 4.07 & $< 0.001$ \\
		XGBoost - SARIMAX  & -0.148 & [-0.197, -0.101] & 5.88 & $< 0.001$ \\
		
		\midrule
		\multicolumn{4}{l}{\textit{All cells}} \\
		SARIMAX - OLS & 0.104 & [0.070, 0.142] & 5.76 & $< 0.001$ \\
		XGBoost - OLS & -0.025 & [-0.040, -0.010] & 3.34 & 0.0011 \\
		XGBoost - SARIMAX & -0.129 & [-0.169, -0.093] & 6.73 & $< 0.001$ \\
...
For non-seasonal cells, every comparison is significant, but the effect sizes differ considerably. SARIMAX gap with OLS is significant at 0.107 ($p < 0.001$), meaning SARIMAX underperformance is most pronounced on non-seasonal cells. XGBoost's advantage over OLS is smaller but still clearly significant at 0.040 ($p < 0.001$), with a confidence interval ([-0.059, -0.021]) that stays comfortably below zero, so unlike the seasonal case, this is a real, if modest, edge for XGBoost. Combined, SARIMAX trails XGBoost by 0.148 ($p < 0.001$), the largest effect size in the entire table. \\

The all-cells results mirror the non-seasonal pattern rather than the seasonal one, which makes sense given non-seasonal cells make up the majority of the dataset. XGBoost retains a significant edge over OLS (0.025, $p = 0.0011$), and both significantly outperform SARIMAX. \\

Overall, the testing changes the conclusion in one important way, the apparent OLS advantage on seasonal cells in Table~\ref{tab:final_model_comparison} is not statistically supported, OLS and XGBoost should be treated as equivalent there. Everywhere else, the gaps are real, SARIMAX is reliably the weakest model in every split, and XGBoost holds a genuine, if small, edge over OLS specifically on non-seasonal and combined data. \\
...
\begin{table}[htbp]
	\centering
	\caption{Effect of global vs group-specific SARIMAX and OLS models}
