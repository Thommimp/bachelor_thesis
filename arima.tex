...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.9\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{figures/arima_post_non_seasonal_order_2.png}
        \caption{Non seasonal Orders}
    ...
    \begin{subfigure}[b]{0.9\linewidth}
        ...
        \caption{Global Arima Orders}
        \label{fig:arima_non_seasonal_post_order}
    \end{subfigure}

    
    
    \caption{Arima coefficients post feature selection}
    \label{fig:post_order_arima}
\end{figure}


The seasonal ARIMA configuration produced 3 orders that occurred with equal frequency of 4 during the selection process. These tied configurations were:
\[
(1,0,0)(0,0,0,7), \quad (3,0,1)(1,0,1,7), \quad (2,0,1)(1,0,1,7)
\]

Since multiple configurations achieved the same selection frequency, each candidate model was evaluated directly using RMSE on the validation set.

The resulting performance was:

\begin{itemize}
    \item Model 1: RMSE = 1.5291
    \item Model 2: RMSE = 1.5649
    \item Model 3: RMSE = 1.5644
\end{itemize}
...
This indicates that a simpler non-seasonal structure performed slightly better than more complex seasonal alternatives, suggesting that seasonal effects are present but not strongly dominant in the selected cells.

Both the non-seasonal and global model got the same order before and after adding features so there RMSE ended up at 1.54 with an order of (0,1,1)(0,0,0,7)
