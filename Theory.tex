...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/cell_towers.png}
    \caption{Cellular network architecture showing base stations and user devices.}
...
To understand how this connectivity is actually achieved at the radio level, the following sections build up from the bottom, starting with the modulation techniques used to transmit data in the network cells. \\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/prb_grid_color.png}
    \caption{PRB Time and Frequency resource grid, showing how time slots are divided between users}
    \label{fig:prb_grid}
\end{figure}
...
Cellular network traffic originates from individual user sessions, where a session referes to any period of data exchange between a users device and the network cell. These session can vary widely in terms of duration, data volume, uplink/downlink depeding on the application.\\

 At any given time, the traffic observed at the cell level is the sum of all active user sessions within that cell. This means that cell level traffic reflects the combined behavior of many independent users rather than a single transmission pattern. As a result, traffic at the network level follows human activity patterns. Users do not generate sessions uniformly over time, but instead concentrate activity around daily routines such as working hours and commuting. This leads to clear daily patterns, with high load during the day and lower activity during the night \cite{diverse_traffic_patterns}. However, the exact shape of these patterns is not uniform across all cells, but instead depends on the type of activity that dominates a given area. \\

Different geographical envirements produce struturally distint traffic patterns, both daily and weekly. Therefor sites can be grouped into different sections such as resident, transport, office and entertainment \cite{wang2015understanding}. \\

Residential areas, where people primarily live, typically show low traffic during working hours and build toward a clear peak in the evening, around 9PM, once residents return home for the day. Transport areas typically show two distinct peaks, one in the morning and one in the late afternoon, corresponding to the start and end of the workday commute. Office areas show roughly the opposite pattern to residential areas, peaking around midday before declining through the afternoon. Entertainment areas show a single peak throughout the day, timing of this peak changes when its weekday vs weekend \cite{wang2015understanding}. \\

On a weekly basis, transport and office areas show noticeably higher traffic on weekdays compared to weekends, consistent with their dependence on commuting and work activity. Residential and entertainment areas, by contrast, show comparable traffic levels between weekdays and weekends, suggesting their usage is less tied to the standard work week \cite{wang2015understanding}. \\

This user activity forms the basis of the measurements collected by the network. The base station continuously records and summarizes this traffic data into cell level indicators, which are then reported as Performance Management (PM) counters at fixed time intervals. \\
